...
# -*- coding: utf-8 -*-
"""
Created on Tue May 30 20:40:19 2017
...
dataset = dataset.drop(['SEX', 'EDUCATION', 'MARRIAGE'], 1)
#DATA BINNING
#dataset.describe()
...
xx = imputer.fit_transform(dataset.iloc[:, 2])
dataset.iloc[:, 2] = xx.flatten()
...
# Remove Customer ID
#X = np.delete(X, 0, 1)
dataset = dataset.drop(['CUST_COD'],1).values
...
models = {
	...
	'AdaBoost' : AdaBoostClassifier(n_estimators=100, random_state=0, base_estimator=DecisionTreeClassifier(criterion='gini', random_state=0)),
     ...
     'XGBClassifier' : XGBClassifier(n_estimators = 100, max_depth = 4, learning_rate = 0.02, gamma = 10,reg_lambda = 10, subsample = 0.5),
     #54.92 reg_lamba = 10 gamma 10 max_depth = 4, n_estimat = 100 l_rate = 0.02
...
#parameters = [
#	             {'gamma' : np.arange(0.0001, 0.1, 10) 
#                  } 
#              ]
#
#grid_search = GridSearchCV(estimator=classifier, param_grid=parameters, scoring='f1', cv=2, n_jobs=-1) # n_jobs = -1 if you use this on large dataset
